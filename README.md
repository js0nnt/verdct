...
- **Theme** — one neutral palette in `src/content/theme.ts` and the Tailwind config, with a single accent. Verdct's own floating surfaces (popover, comparison panel) and the popup follow `prefers-color-scheme`. Badges stay light whatever the OS theme: they sit inside ASU's page, which is always light, and a dark chip in a white results table would read as broken rather than as dark mode.
